...
def normalize_isbn13(raw: str) -> str:
    ...
    # Digits alternate weight 1, 3, 1, 3, ... across the first twelve positions.
    weighted_sum = sum(digit * (3 if index % 2 else 1) for index, digit in enumerate(digits[:12]))
...
class OrderCreate(BaseModel):
    ...
    def validate_items(cls, items: List[OrderItemIn]) -> List[OrderItemIn]:
        ...
        if len(book_ids) != len(set(book_ids)):
            raise ValueError("the same book_id may not appear twice in one order")
        return items
