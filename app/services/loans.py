...
def calculate_late_fee(due_at: datetime, returned_at: datetime, price_cents: int) -> int:
    ...
    if seconds_late <= 0:
        return 0
    days_late = math.ceil(seconds_late / timedelta(days=1).total_seconds())
...
def create_loan(db: Session, data: LoanCreate, now: datetime) -> LoanOut:
    ...
    # Everything below reasons about the member's currently unreturned loans, so it is
    # fetched once and reused for the overdue check, the duplicate-book check and the limit.
    unreturned = list(
        db.scalars(
...
def list_member_loans(
    ...
    get_member(db, member_id)  # raises 404 if the member doesn't exist

    loans = db.scalars(select(Loan).where(Loan.member_id == member_id).order_by(Loan.id))
    outs = [to_loan_out(loan, now) for loan in loans]
    if status is not None:
        outs = [loan for loan in outs if loan.status == status]
    return outs