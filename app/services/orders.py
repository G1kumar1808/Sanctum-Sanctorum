...
def create_order(db: Session, data: OrderCreate, now: datetime) -> Order:
    ...
    # Load every book up front, keyed by id, so later loops don't hit the database again
    # and so a missing book is caught before any restricted/stock check runs.
    books = {}
    ...
    # Stock is checked for every item before anything is changed, so a shortfall on the
    # last item still leaves the first items' stock untouched.
    for item in data.items:
